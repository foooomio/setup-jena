...
var CDN_PAGE_URL = "https://dlcdn.apache.org/jena/binaries/";
var ARCHIVE_PAGE_URL = "https://archive.apache.org/dist/jena/binaries/";
async function getLatest() {
  const [info2] = await getAvailableList(CDN_PAGE_URL);
  if (!info2) {
...
async function getSatisfied(input) {
  ...
  const info2 = list.find(
    (candidate) => (0, import_satisfies.default)(candidate.version, search)
  );
  if (!info2) {
...
async function getAvailableList(url) {
  ...
  for (const [, filename, version2] of html.matchAll(regexp)) {
    if (filename && version2) {
      list.push({ version: version2, downloadUrl: url + filename });
    }
  }
  if (!list.length) {
    throw new Error("Could not find any available versions.");
  }
  return list.sort((a, b) => (0, import_compare.default)(b.version, a.version));
}

// src/setup-jena.ts
async function installJena(version2, downloadUrl) {
  const archivePath = await tc.downloadTool(downloadUrl);
  const flags = ["xz", "--strip=1"];
  const extractedPath = await tc.extractTar(archivePath, void 0, flags);
  const cachedPath = await tc.cacheDir(extractedPath, "jena", version2);
  return cachedPath;
}
async function run() {
  const version2 = core.getInput("jena-version");
  const isLatest = !version2 || version2 === "latest" || version2 === "*";
  const jena = isLatest ? await getLatest() : await getSatisfied(version2);
  ...
  core.addPath(import_path.default.join(jenaPath, "bin"));
